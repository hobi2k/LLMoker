...
- 프로그램 시작 로고 음원: `llmoker/game/audio/logo.ogg`
- 프로그램 시작 인트로 음원: `llmoker/game/audio/intro.ogg`
- 프로그램 시작 오프닝 음원: `llmoker/game/audio/opening.ogg`
- 프로그램 시작 인트로 영상: `llmoker/game/gui/intro.webm`
- 메인 메뉴 BGM: `llmoker/game/audio/main.ogg`
- 포커 플레이 BGM: `llmoker/game/audio/game.ogg`

시작 영상 구간은 `webm` 내장 오디오 대신 `ogg`를 따로 재생하고, 메뉴/게임 BGM도 `ogg` 파일을 Ren'Py `music` 채널에서 루프로 재생한다. 이 구조는 다른 Windows PC에서의 오디오 호환성을 높이기 위해 유지한다.
...
- 여기서 먼저 `audio/logo.ogg`와 `gui/logo_silent.webm`를 함께 재생한다.
- 이어서 안내 문구를 잠깐 띄운다.
- 짧은 검은 화면 뒤 `audio/intro.ogg`와 `gui/intro.webm`를 함께 재생한다.
- 짧은 검은 화면 뒤 `audio/opening.ogg`와 `gui/openingcinema_silent.webm`를 함께 재생한다.
...
즉, 메인 메뉴나 메인 메뉴 기반 설정 화면으로 돌아오면 `audio/main.ogg`가 다시 걸린다.
...
즉, 포커 테이블로 들어오거나 게임 세이브를 복원하면 `audio/game.ogg`가 재생된다.
