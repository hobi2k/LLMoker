...
로컬 모델의 가중치를 직접 업데이트하지 않고, 결과와 피드백을 다음 추론 문맥에 다시 넣어 행동 정책을 수정하는 방식이다.
...
- 라운드 결과를 텍스트 피드백으로 만든다.
- 그 피드백을 메모리에 저장한다.
- 다음 행동 프롬프트, 카드 교체 프롬프트, 대사 프롬프트에 다시 넣는다.
