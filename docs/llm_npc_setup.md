...
- 행동은 합법 행동 하나만 고른다.
- 카드 교체는 인덱스 목록만 고른다.
- 회고는 LLM이 `short_term`, `long_term`, `strategy_focus`를 직접 작성한다.
- 런타임은 `self`, `opponent` 역할 용어와 필수 필드만 정규화한다.
- 다만 `policy_loop.py`는 승패, 족보, 폴드 종료 여부와 충돌하는 회고를 저장하지 않는다.
