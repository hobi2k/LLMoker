...
- Ren'Py와 결합했을 때 규칙 흐름과 행동 피드백을 화면에 즉시 보여주기 쉽다.
...
- 현재 음악 자산은 `ACE-Step` 기반 생성본을 사용
- 음성/TTS 자산 제작 기록은 `CosyVoice` 기준으로 관리한다
...
- 기본 런타임 모델은 `Qwen3-4B-Instruct-2507`이고, 행동/드로우/회고는 `Qwen-Agent + FnCallAgent` 경로를 유지한다.
- 베팅 라운드에서는 콜 금액이 0이면 `check`, `bet`만 합법 행동으로 둔다.
- `fold`는 상대 베팅이나 레이즈로 실제 콜 금액이 생겼을 때만 합법 행동으로 둔다.
- 로컬 모델 폴더 내부 파일은 수정하지 않는다.
- QA는 실제 플레이 검증과 함께 `Codex`, `Claude Code`를 활용한 코드/흐름 검수를 병행한다.
...
이 프로젝트에서 용어는 아래처럼 통일한다.

- `체크`
  - 현재 테이블 베팅이 0칩일 때 그냥 넘긴다.
- `베팅 10칩`
  - 현재 테이블 베팅이 0칩일 때 처음으로 10칩을 건다.
- `콜 n칩`
  - 현재 테이블 베팅을 맞추기 위해 부족한 금액 `n칩`을 낸다.
- `총 n칩 레이즈`
  - 현재 콜 금액에 고정 베팅 단위 10칩을 더해 총액 `n칩`까지 올린다.
- `폴드`
  - 그 라운드를 즉시 포기한다.
...
- 첫 번째 베팅은 항상 플레이어가 먼저 시작한다.
- 첫 번째 베팅이 끝나면 플레이어가 먼저 드로우를 선택하고, 그 다음 NPC가 드로우를 수행한다.
