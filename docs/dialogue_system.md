...
현재는 이벤트 훅은 Ren'Py에 있고, `LLM NPC` 모드에서는 Qwen-Agent tool calling으로 대사를 만든다.
...
- 라운드 종료 시에는 먼저 `lost.webm` 또는 `win.webm` 위에서 대사가 나오고, 그 다음 결과 패널이 열린다.
...
- `build_dialogue_prompt(...)`
  - 실제 대사 생성용 tool-calling 프롬프트를 만든다.
- `LocalLLMAgent.generate_dialogue(...)`
  - 실제로 Qwen-Agent 워커를 호출한다.
...
2. 현재 포커 상태 수집
3. 캐릭터 기억과 최근 로그 수집
4. `backend/llm/prompts.py`로 프롬프트 구성
5. `backend/llm/tools.py`가 Qwen-Agent용 포커 도구를 제공
6. `backend/llm/agent.py`가 워커에 대사 생성 요청
7. Qwen-Agent가 `get_public_state`, `get_recent_log`, `get_memory`를 필요에 따라 호출
8. 성공 시 생성 대사 출력
