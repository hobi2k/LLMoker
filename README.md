...
- 행동, 카드 교체, 대사, 정책 회고는 모두 같은 transformers 런타임에서 처리
...
- 대사 생성은 `Qwen-Agent` 경로를 유지하되, 최근 공개 사건을 짧은 사건 문장으로 다시 써서 입력합니다.
- 대사 프롬프트는 `너는 포커를 플레이하는 캐릭터 사야다.`로 시작해, 역할 지시를 먼저 고정합니다.
- 게임 레벨 태스크는 `행동 선택`, `카드 교체 판단`, `심리전 대사 생성`, `라운드 회고 및 전략 업데이트` 네 가지입니다.
