...
- LLM 행동 선택, 카드 교체 판단 연결
- 라운드 종료 후 정책 회고도 LLM이 생성하고 다음 판단 문맥에 반영
- 게임 레벨 작업은 `backend/llm/tasks.py`에서 행동 선택, 카드 교체 판단, 라운드 회고로 분리
- 공개 정보와 비공개 정보 분리
...
- 라운드 종료 시에는 `lost.webm` 또는 `win.webm` 위에서 행동/단계 나레이션이 먼저 나오고, 그 다음 결과 패널이 열립니다.

## 오디오 자산

- 메인 메뉴 BGM: `llmoker/game/audio/main.ogg`
- 포커 게임 BGM: `llmoker/game/audio/game.ogg`
- 시작 영상 오디오는 `logo.ogg`, `intro.ogg`, `opening.ogg`로 분리해 재생
...
- Linux에서는 clone 직후 `.venv`가 없으면 먼저 `llmoker/.venv`를 생성합니다.
- Windows에서는 외부 Python을 전제로 하지 않고, 첫 실행 시 `llmoker/.runtime/py311-windows-x86_64/` 아래에 임베디드 Python 3.11 런타임을 자동 준비합니다.
- 런타임에 `pip`가 없으면 Linux는 `ensurepip`, Windows는 `get-pip.py`로 먼저 복구합니다.
...
- LLM 런타임은 플랫폼별 별도 Python으로 실행합니다.
  - Linux: `llmoker/.venv/bin/python`
  - Windows: `llmoker/.runtime/py311-windows-x86_64/python.exe`
...
- `LLM NPC` 모드에서는 행동/드로우/정책 회고 실패를 스크립트봇이나 안전 행동으로 숨기지 않습니다.
...
- 모델 원본 파일은 수정하지 않습니다.
- `Qwen3-4B-Instruct-2507`를 기본 런타임 모델로 사용하며, 행동 JSON과 정책 회고만 최종 출력으로 읽습니다.
- 태스크별 생성 상한은 문맥 길이와 JSON 안정성을 함께 고려해 유지합니다. 현재 기준으로 행동/드로우는 `64`, 정책 회고는 `384` 토큰입니다.
...
```bash
cd llmoker
./.venv/bin/python scripts/check_raw_inference.py --mode sanity
```

Windows에서는 위 수동 점검 명령 대신 게임 실행 시 `llmoker/.runtime/py311-windows-x86_64/`가 자동 준비되는지를 먼저 확인하면 됩니다.
...
- Windows는 외부 Python 없이도 프로젝트 내부 `.runtime/py311-windows-x86_64/`를 자동 준비하는 self-contained 부트스트랩 구조를 사용합니다.
