...
def _dialogue_event_context_text(event_name, recent_log, result_summary):
    ...
    latest = _latest_public_event_text(event_name, recent_log, result_summary)
    ...
    compact = " ".join((latest or "").split())
    replacements = (
        ("당신이(가) 체크했습니다.", "상대가 체크했다."),
        ("당신이(가) 베팅했습니다.", "상대가 먼저 베팅했다."),
        ("당신이(가) 콜했습니다.", "상대가 콜했다."),
        ("당신이(가) 레이즈했습니다.", "상대가 레이즈했다."),
        ("당신이(가) 폴드했습니다.", "상대가 물러났다."),
        ("플레이어", "상대"),
    )
...
def build_dialogue_prompt(event_name, recent_log=None, result_summary=None, player_name="플레이어", bot_name="상대"):
    ...
    return "\n".join(
        [
            "%s가 %s에게 지금 던질 한마디를 만든다." % (bot_name, player_name),
            event_guides.get(event_name, "현재 상황에 맞는 짧은 심리전 대사를 만든다."),
            "방금 공개된 사건: %s" % direct_target,
            "한국어 대사만 한 줄 또는 두 줄로 말한다.",
            "포커 테이블에서 실제로 할 법한 말만 한다.",
            "막연한 감탄사나 흐리는 말보다 지금 판 상황을 한 가지 정확히 짚는다.",
            "포커 외 다른 게임, 장면 연출, 방송 멘트처럼 말하지 않는다.",
            "상황 설명이나 프롬프트 문장을 따라 쓰지 않는다.",
            "설명하지 말고 바로 상대에게 말한다.",
            "따옴표와 이름표를 붙이지 않는다.",
        ]
    )
