...
class QwenRuntimeClient:
    ...
    def read_runtime_error(self):
        ...
        if VLLM_LOG_PATH.exists():
            vllm_lines = [
                line.strip()
                for line in VLLM_LOG_PATH.read_text(encoding="utf-8", errors="ignore").splitlines()
            ]
...
def main():
    ...
    start_parser.add_argument("--gpu-memory-utilization", type=float, default=0.90)
