...
def strip_action_or_reason_prefix(text, legal_actions=None):
    """
    행동/이유 문자열 앞에 붙은 형식용 접두를 걷어낸다.

    Args:
        text: 모델이 반환한 원본 문자열이다.
...
def normalize_reason_text(text, fallback, legal_actions=None):
    ...
    Args:
        ...
        legal_actions: 이유 앞에 붙은 행동 토큰을 제거할 때 참고할 허용 행동 목록이다.
...
class QwenRuntime:
    ...
    def read_vllm_error(self):
        ...
        lines = [line.strip() for line in VLLM_LOG_PATH.read_text(encoding="utf-8", errors="ignore").splitlines()]
        priority_markers = (
            "ValueError:",
            "RuntimeError:",
