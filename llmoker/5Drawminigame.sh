...
if ! (cd "$ROOT" && "$RUNNER_PYTHON" -m backend.llm.model_bootstrap); then
    ...
    echo "공식 모델: https://huggingface.co/Qwen/Qwen3-4B-Instruct-2507-FP8"
    echo "배치 위치: $ROOT/models/llm/qwen3-4b-instruct-fp8"
    echo "자동 다운로드를 건너뛰려면 LLMOKER_SKIP_MODEL_DOWNLOAD=1 로 실행하세요."
    exit 1
fi

if ! (cd "$ROOT" && "$RUNNER_PYTHON" -m backend.llm.client start \
    --model-path "$ROOT/models/llm/qwen3-4b-instruct-fp8" \
    --model-name "Qwen3-4B-Instruct-2507-FP8" \
    --runtime-python "$RUNNER_PYTHON" \
    --device "${LLM_DEVICE:-auto}" \
    --host "127.0.0.1" \
    --port "${LLM_RUNTIME_PORT:-8011}" \
    --vllm-port "${LLM_VLLM_PORT:-8000}" >"$RUNTIME_START_LOG" 2>&1); then
    echo
